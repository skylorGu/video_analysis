...
function copySummaryContent(button) {
    ...
    try {
        ...
        if (summaryContainer) {
            ...
            console.log('提取的文本长度:', textToCopy.length);
            console.log('文本内容预览:', textToCopy.substring(0, 100));
        }
        ...
        if (textToCopy && textToCopy.trim()) {
            ...
            link.href = url;
            link.download = '内容摘要_' + new Date().toISOString().slice(0, 19).replace(/:/g, '-') + '.txt';
            ...
            button.innerHTML = '<i class="bi bi-check me-1"></i>已保存!';
    ...
    function sendMessage() {
        ...
        setTimeout(() => {
            ...
            .catch(error => {
                ...
                // 使用打字效果显示错误信息
                typeText('抱歉，我现在无法回答您的问题。', typingText, typingCursor, cursorInterval);
            });
        }, 2000);
    }
    ...
    videoUpload.addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            .catch(error => {
                ...
                // 如果上传失败，使用模板数据
                updateLoadingStatus('连接服务器失败，使用演示数据...');
...
const templateData = {
  ...
  - 在实际开发中，如何评估和改进LLM生成的文本质量？`,
    ...
    setTimeout(function() {
        ...
        console.log('延迟查找复制按钮:', copyBtn);
        
        if (copyBtn) {
            console.log('成功绑定复制按钮事件');
            copyBtn.onclick = function(e) {
                ...
                if (textToCopy && textToCopy.trim()) {
                    try {
                        ...
                        copyBtn.innerHTML = '<i class="bi bi-check me-1"></i>已保存!';
            ...
            console.log('复制按钮未找到，尝试事件委托');
            // 使用事件委托作为备选方案
            document.addEventListener('click', function(e) {
                if (e.target && e.target.id === 'copy-summary') {
                    console.log('通过事件委托捕获到复制按钮点击');
                    e.preventDefault();
                    ...
                    if (summaryContainer) {
                        ...
                        if (textToCopy.trim()) {
                            ...
                            e.target.innerHTML = '<i class="bi bi-check me-1"></i>已保存!';
                            setTimeout(() => {
                                e.target.innerHTML = '<i class="bi bi-clipboard me-1"></i>复制';
                            }, 2000);
                        }
                    }
                }
